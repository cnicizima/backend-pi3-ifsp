...
async function main() {
    ...
    for (let i = 0; i < 20; i++) {
        const produto = await prisma.produto.create({
            data: {
                ...
                gustativo: faker.helpers.arrayElement(descricoesGustativas), // Escolhe uma descrição gustativa realista
                olfativo: faker.helpers.arrayElement(descricoesOlfativas), // Escolhe uma descrição olfativa realista
                regiao: faker.helpers.arrayElement(regioesVinicolas), // Escolhe uma região vinícola realista
                amadurecimento: `${faker.number.int({ min: 6, max: 24 })} meses em barris de carvalho`,
                analises: faker.helpers.arrayElement(descricoesAnalises), // Escolhe uma análise realista
                uvas: faker.helpers.arrayElement(uvasReais), // Escolhe uma uva real
    ...
    for (let i = 0; i < 10; i++) {
        ...
        for (let j = 0; j < 2; j++) {
            const endereco = await prisma.endereco.create({
                data: {
                    ...
                    bairro: faker.helpers.arrayElement(['Centro', 'Jardim', 'Vila', 'Residencial']),
                },
            });

            console.log(`Endereço criado para o usuário ${usuario.cpf}: ${endereco.idEndereco}`);
            ...
            for (let k = 0; k < 2; k++) {
                const pedido = await prisma.pedido.create({
                    data: {
                        ...
                        valorTotal: faker.number.float({ min: 50, max: 500, precision: 0.01 }),
                        status: faker.helpers.arrayElement(['Pendente', 'Pago', 'Cancelado']),
                        idCupom: faker.helpers.arrayElement(cupons).idCupom,
                    },
                });

                console.log(`Pedido criado para o usuário ${usuario.cpf}: ${pedido.idPedido}`);
                ...
                console.log(`Pagamento criado para o pedido ${pedido.idPedido}: ${pagamento.idPagamento}`);
                ...
                for (let l = 0; l < 3; l++) {
                    ...
                    console.log(`Item de pedido criado para o pedido ${pedido.idPedido}: ${pedidoProduto.idPedidoProduto}`);
                }
            }
        }
        ...
        for (let m = 0; m < 3; m++) {
            const avaliacao = await prisma.avaliacao.create({
                data: {
                    ...
                    avaliacao: faker.number.float({ min: 1, max: 5, precision: 0.1 }),
                    conteudo: faker.lorem.sentence(),
                    dataCriacao: faker.date.recent().toISOString(),
                },
            });

            console.log(`Avaliação criada para o usuário ${usuario.cpf}: ${avaliacao.idAvaliacao}`);
        }
        ...
        for (let n = 0; n < 3; n++) {
            ...
            console.log(`Favorito criado para o usuário ${usuario.cpf}: ${favorito.idFavorito}`);
        }
        ...
        for (let o = 0; o < 2; o++) {
            ...
            console.log(`Mensagem criada para o usuário ${usuario.cpf}: ${mensagem.idMensagem}`);
        }
    }
